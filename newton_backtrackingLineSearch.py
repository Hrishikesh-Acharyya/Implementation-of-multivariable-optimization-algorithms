...
def hessianInverse(x1,x2):
    return np.linalg.inv(hessian(x1, x2))
...
def calcAlpha(x):
    ...
    hessInv = hessianInverse(x[0], x[1])
    direction = -hessInv @ grad
    while f(*(x + alpha * direction)) > f(x[0], x[1]) + beta * alpha * np.dot(grad, direction):
        alpha *= rho
        if alpha < 1e-8:
...
def newton(starting_point, tolerance = 1e-8):
  ...
  while(True):
     ...
     alpha = calcAlpha(x0)
     hessInv = hessianInverse(x0[0], x0[1])
...
result, path_x1, path_x2, path_f = newton(starting_point)
